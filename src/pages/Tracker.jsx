...
import { archiveLowFit, updateOpportunity } from '../lib/api.js';
...
const STATUSES = ['all', 'discovered', 'queued', 'approved', 'apply_pack_generated', 'ready_to_apply', 'applied', 'interviewing', 'offer', 'rejected', 'archived_low_fit', 'stale', 'ghosted'];
...
export default function Tracker() {
  ...
  const [archivingLowFit, setArchivingLowFit] = useState(false);
  ...
  const lowFitActiveCount = useMemo(() =>
    state.opportunities.filter(o =>
      (o.fit_score || 0) < 50 &&
      !['applied','interviewing','offer','rejected','ghosted','withdrawn','archived_low_fit'].includes(o.status)
    ).length,
  [state.opportunities]);
  ...
  const handleArchiveLowFit = async () => {
    if (!lowFitActiveCount) return;
    const ok = window.confirm(`Archive ${lowFitActiveCount} active low-fit roles below 50? They will move to the Archived Low Fit bucket, not be permanently deleted.`);
    if (!ok) return;
    setArchivingLowFit(true);
    try {
    ...
    <div>
      <div className="page-heading">
        <div>
          ...
          <p className="section-sub">Full pipeline view, searchable and sorted by readiness.</p>
        </div>
        <button
          className="btn btn-secondary btn-sm"
          disabled={archivingLowFit || lowFitActiveCount === 0}
          onClick={handleArchiveLowFit}
          title="Move active roles with fit score below 50 into a reversible archive bucket"
        >
          Archive low fit ({lowFitActiveCount})
        </button>
      </div>
              ...
              {opps.map(o => (
                <tr key={o.id} style={{ borderBottom: '1px solid var(--gray-100)' }}>
                  ...
                  <td style={{ padding: '10px 12px' }}>
                    ...
                    >
                      {['discovered','queued','approved','needs_apply_url','apply_pack_generated','ready_to_apply','applied','follow_up_1','follow_up_2','interviewing','offer','rejected','archived_low_fit','stale','ghosted','withdrawn'].map(s => (
